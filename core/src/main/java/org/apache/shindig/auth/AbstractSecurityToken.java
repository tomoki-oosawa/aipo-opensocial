 ...
 * isExpired() method and getters/setters for nearly every field of the token.
 *
 * @since 2.0.0
 */
public abstract class AbstractSecurityToken implements SecurityToken {
  ...
  public enum Keys {
    OWNER("o") {
      ...
      @Override
      public void loadFromMap(AbstractSecurityToken token,
    ...
    /**
     * Gets the {@link String} value from the {@link SecurityToken} using the
     * getter that this {@link Keys} is bound to.
     *
     * @param token
     *          The token to get the value from.
     * @return The value
     */
    public abstract String getValue(SecurityToken token);

    /**
     * Loads from the map the value bound to this {@link Keys} and sets it on
     * the {@link SecurityToken}
     *
     * @param token
     *          The token to insert set the value on.
     * @param map
     *          The map to read the value from.
     */
    public abstract void loadFromMap(AbstractSecurityToken token,
        Map<String, String> map);
  }
   ...
   *
   * @param timeSource
   *          The new {@link TimeSource} for this token to use.
   * @return This object.
   */
  @VisibleForTesting
   ...
   *
   * @param tokenTTL
   *          the time to live (in seconds) of the token
   * @return This security token.
   */
  protected AbstractSecurityToken setExpires(int tokenTTL) {
    this.tokenTTL = tokenTTL;
    return setExpiresAt((getTimeSource().currentTimeMillis() / 1000)
      + getMaxTokenTTL());
  }
   ...
   *
   * @param expiresAt
   *          When this token expires, in seconds since epoch.
   * @return This security token.
   */
   ...
   * that handle additional keys not contained in {@link Keys} should override
   * and supplement the functionality of this method.
   *
   ...
   * Override this method only if you are internal token that doesn't get
   * serialized via {@link SecurityTokenCodec#encodeToken(SecurityToken)}, e.g.,
   * OAuth state tokens. For all other cases, the SecurityTokenCodec will handle
  ...
  /**
   * This method will govern the effectiveness of the protected {@link #toMap()}
   * and {@link #loadFromMap(SecurityToken, Map)} helper methods. <br>
   * <br>
   * If your implementation throws an exception on any of the get methods, you
   * should not include the associated key here, and those values should be
   * handled in an overridden implementation of {@link #toMap()} if they might
