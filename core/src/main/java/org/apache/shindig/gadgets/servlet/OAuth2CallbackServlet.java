...
public class OAuth2CallbackServlet extends InjectedServlet {
  ...
  private static final Logger LOGGER = Logger
    .getLogger(OAuth2CallbackServlet.LOG_CLASS);
  ...
  private static final String RESP_ERROR_BODY =
    "<!DOCTYPE HTML PUBLIC \"-//W3C//DTD HTML 4.01 Transitional//EN\" "
      + "\"http://www.w3.org/TR/html4/loose.dtd\">\n"
      + "<html>\n"
      + "<head>\n"
  ...
  protected void doGet(final HttpServletRequest request,
    ...
    try {
      ...
      if (encRequestStateKey == null) {
        if (error != null) {
          OAuth2CallbackServlet.sendError(
            error,
            "encRequestStateKey is null",
          ...
          OAuth2CallbackServlet.sendError(
            OAuth2Error.CALLBACK_PROBLEM,
            "OAuth2CallbackServlet requestStateKey is null.",
            "",
            "",
            null,
            resp,
      ...
      final OAuth2CallbackState state =
        new OAuth2CallbackState(this.stateCrypter, encRequestStateKey);

      accessor = this.store.getOAuth2Accessor(state);
      if (accessor != null) {
        accessor.setRedirecting(true);
        OAuth2GadgetContext context =
          new OAuth2GadgetContext(Uri.parse(state.getGadgetUri()), state
            .getScope());
        GadgetSpecFactory specFactory =
          injector.getInstance(GadgetSpecFactory.class);
        GadgetSpec spec = specFactory.getGadgetSpec(context);
        OAuth2Spec oAuth2Spec = spec.getModulePrefs().getOAuth2Spec();
        if (oAuth2Spec != null) {
          OAuth2Service service =
            oAuth2Spec.getServices().get(state.getServiceName());
          if (service != null) {
            EndPoint tokenUrl = service.getTokenUrl();
            if (tokenUrl != null) {
              accessor.setTokenUrl(service.getTokenUrl().getUrl().toString());
            }
            EndPoint authorizationUrl = service.getAuthorizationUrl();
            if (authorizationUrl != null) {
              accessor.setAuthorizationUrl(service
                .getAuthorizationUrl()
                .getUrl()
                .toString());
            }
      ...
      if (accessor == null || !accessor.isValid() || accessor.isErrorResponse()) {
        ...
        if (accessor != null) {
          message =
            accessor.isValid()
              ? "OAuth2CallbackServlet accessor isErrorResponse "
      ...
      for (final AuthorizationEndpointResponseHandler authorizationEndpointResponseHandler : this.authorizationEndpointResponseHandlers) {
        if (authorizationEndpointResponseHandler.handlesRequest(
          accessor,
          request)) {
          final OAuth2HandlerError handlerError =
            authorizationEndpointResponseHandler.handleRequest(
              accessor,
              request);
      ...
      if (!foundHandler) {
        OAuth2CallbackServlet
          .sendError(
            OAuth2Error.NO_RESPONSE_HANDLER,
      ...
      OAuth2CallbackServlet.sendError(
        OAuth2Error.CALLBACK_PROBLEM,
        "Exception occurred processing redirect.",
        "",
        "",
        accessor,
        resp,
  ...
  private static void sendError(final OAuth2Error error,
      final String contextMessage, final String description, final String uri,
      final OAuth2Accessor accessor, final HttpServletResponse resp,
      final Throwable t, final boolean sendTraceToClient) throws IOException {

    OAuth2CallbackServlet.LOGGER.warning(OAuth2CallbackServlet.LOG_CLASS
      + " , callback error "
      + error
      + " -  "
      + contextMessage
    ...
    if (accessor != null) {
      ...
      if (sendTraceToClient) {
        errorResponse =
          String.format(OAuth2CallbackServlet.RESP_ERROR_BODY, error
            .getErrorCode(), error.getErrorDescription(description), uri);
      } else {
        errorResponse =
          String.format(OAuth2CallbackServlet.RESP_ERROR_BODY, error
            .getErrorCode(), "", "");
  ...
  @Inject
  public void setOauth2MessageProvider(
