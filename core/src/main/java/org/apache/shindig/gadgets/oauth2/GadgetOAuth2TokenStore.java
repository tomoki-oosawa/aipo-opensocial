...
public class GadgetOAuth2TokenStore {
  ...
  private final static String LOG_CLASS = GadgetOAuth2TokenStore.class
    .getName();
  ...
  private static class OAuth2SpecInfo {
    ...
    public OAuth2SpecInfo(final String authorizationUrl, final String tokenUrl,
        final String scope) {
  ...
  @Inject
  public GadgetOAuth2TokenStore(final OAuth2Store store,
      final GadgetSpecFactory specFactory) {
    this.store = store;
    ...
    if (GadgetOAuth2TokenStore.LOG.isLoggable()) {
      ...
      GadgetOAuth2TokenStore.LOG
        .log("this.specFactory = {0}", this.specFactory);
    }
  }

  private GadgetSpec findSpec(final SecurityToken securityToken,
      final OAuth2Arguments arguments, final Uri gadgetUri)
      throws OAuth2RequestException {
    final boolean isLogging = GadgetOAuth2TokenStore.LOG.isLoggable();
    if (isLogging) {
      GadgetOAuth2TokenStore.LOG.entering(
        GadgetOAuth2TokenStore.LOG_CLASS,
        "findSpec",
    ...
    try {
      final GadgetContext context =
        new OAuth2GadgetContext(securityToken, arguments, gadgetUri);
      ret = this.specFactory.getGadgetSpec(context);
    } catch (final IllegalArgumentException e) {
      if (isLogging) {
        GadgetOAuth2TokenStore.LOG.log("Error finding GadgetContext "
          + gadgetUri.toString(), e);
      }
      throw new OAuth2RequestException(
        OAuth2Error.GADGET_SPEC_PROBLEM,
        gadgetUri.toString(),
        e);
    ...
    if (isLogging) {
      ...
      if (ret == null) {
        GadgetOAuth2TokenStore.LOG.exiting(
          GadgetOAuth2TokenStore.LOG_CLASS,
          "findSpec",
          null);
      } else {
  ...
  public OAuth2Accessor getOAuth2Accessor(final SecurityToken securityToken,
    ...
    if ((this.store == null) || (gadgetUri == null) || (securityToken == null)) {
      ...
      final String serviceName =
        arguments != null ? arguments.getServiceName() : "";
      ...
      if (ret == null && arguments != null) {
        ...
        try {
          persistedAccessor =
            this.store.getOAuth2Accessor(securityToken.getAppId(), gadgetUri
              .toString(), serviceName, securityToken.getViewerId(), scope);
        } catch (final GadgetException e) {
          if (isLogging) {
        ...
        if (persistedAccessor == null) {
          ...
          if (persistedAccessor.isAllowModuleOverrides()) {
            ...
            if ((specAuthorizationUrl != null)
              && (specAuthorizationUrl.length() > 0)) {
  ...
  private OAuth2SpecInfo lookupSpecInfo(final SecurityToken securityToken,
    ...
    if (oauthSpec == null) {
      throw new OAuth2RequestException(
        OAuth2Error.LOOKUP_SPEC_PROBLEM,
    ...
    final OAuth2Service service =
      oauthSpec.getServices().get(arguments.getServiceName());
    if (service == null) {
      throw new OAuth2RequestException(
        OAuth2Error.LOOKUP_SPEC_PROBLEM,
    ...
    final OAuth2SpecInfo ret =
      new OAuth2SpecInfo(authorizationUrl, tokenUrl, service.getScope());
