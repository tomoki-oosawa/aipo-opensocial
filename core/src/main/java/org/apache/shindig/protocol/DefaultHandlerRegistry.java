...
public class DefaultHandlerRegistry implements HandlerRegistry {
  ...
  private static final Logger LOG = Logger
    .getLogger(DefaultHandlerRegistry.class.getName());

  // Map service - > method -> { handler, ...}
  private final Map<String, Map<String, SortedSet<RestPath>>> serviceMethodPathMap =
    Maps.newHashMap();

  private final Map<String, RpcInvocationHandler> rpcOperations = Maps
    .newHashMap();
  ...
  public void addHandlers(Set<Object> handlers) {
    for (final Object handler : handlers) {
      ...
      Preconditions.checkState(
        handlerType.isAnnotationPresent(Service.class),
        "Attempt to bind unannotated service implementation %s",
        handlerType.getName());
  ...
  public Set<String> getSupportedRestServices() {
    ...
    for (Map<String, SortedSet<RestPath>> methods : serviceMethodPathMap
      .values()) {
  ...
  private void createRestHandler(Provider<?> handlerProvider, Service service,
    ...
    try {
      ...
      RestInvocationHandler restHandler =
      ...
      LOG.log(Level.INFO, "No REST binding for "
        + service.name()
        + '.'
        + m.getName());
    }
  ...
  private void createRpcHandler(Provider<?> handlerProvider, Service service,
    ...
    try {
      ...
      LOG.log(Level.INFO, "No RPC binding for "
        + service.name()
        + '.'
        + m.getName());
    }
  ...
  static final class RpcInvocationHandler {
    ...
    private RpcInvocationHandler(MethodCaller methodCaller,
        Provider<?> handlerProvider, BeanJsonConverter beanJsonConverter,
    ...
    public Future<?> execute(JSONObject rpc,
        Map<String, FormDataItem> formItems, SecurityToken token,
        BeanConverter converter) {
      RequestItem item;
      try {
        JSONObject params =
          rpc.has("params") ? (JSONObject) rpc.get("params") : new JSONObject();
        item =
          methodCaller.getRpcRequestItem(
            params,
            formItems,
            token,
            beanJsonConverter);
  ...
  static final class RpcInvocationWrapper implements RpcHandler {
    ...
    public Future<?> execute(Map<String, FormDataItem> formItems,
        SecurityToken st, BeanConverter converter) {
  ...
  static final class RestInvocationHandler {
    ...
    private RestInvocationHandler(Operation operation,
        MethodCaller methodCaller, Provider<?> handlerProvider,
    ...
    public Future<?> execute(Map<String, String[]> parameters,
        Map<String, FormDataItem> formItems, SecurityToken token,
        BeanConverter converter) {
      ...
      try {
        item =
          methodCaller.getRestRequestItem(
            parameters,
            formItems,
            token,
            converter,
  ...
  private static class MethodCaller {
     ...
     */
    public MethodCaller(Method method, boolean isRest)
        throws NoSuchMethodException {
      this.method = method;
      ...
      boolean inputIsRequestItem =
        (inputClass != null) && RequestItem.class.isAssignableFrom(inputClass);

      Class<?> requestItemType =
        inputIsRequestItem ? inputClass : BaseRequestItem.class;

      restRequestItemConstructor =
        requestItemType.getConstructor(
          Map.class,
          Map.class,
          SecurityToken.class,
          BeanConverter.class,
    ...
    public RequestItem getRpcRequestItem(JSONObject params,
        Map<String, FormDataItem> formItems, SecurityToken token,
    ...
    private RequestItem getRequestItem(Map<String, String[]> params,
        Map<String, FormDataItem> formItems, SecurityToken token,
        BeanConverter converter, BeanJsonConverter jsonConverter,
        Constructor<?> constructor) {
      try {
        return (BaseRequestItem) constructor.newInstance(
          params,
          formItems,
          token,
          converter,
    ...
    private RequestItem getRequestItem(JSONObject params,
        Map<String, FormDataItem> formItems, SecurityToken token,
        BeanConverter converter, BeanJsonConverter jsonConverter,
        Constructor<?> constructor) {
      try {
      ...
      for (int i = 0; i < Math.min(requestPathParts.length, parts.size()); i++) {
        if (parts.get(i).type == PartType.SINGULAR_PARAM) {
          ...
          parsedParams.put(
            ...
            new String[] { requestPathParts[i] });
        } else if (parts.get(i).type == PartType.PLURAL_PARAM) {
          parsedParams.put(parts.get(i).partName, StringUtils
            .splitPreserveAllTokens(requestPathParts[i], ','));
        }
      }
    ...
    public boolean equals(Object other) {
      if (other instanceof RestPath) {
        ...
        return (this.constCount == that.constCount
          && this.lastConstIndex == that.lastConstIndex && Objects.equal(
          this.operationPath,
          that.operationPath));
      }
      return false;
