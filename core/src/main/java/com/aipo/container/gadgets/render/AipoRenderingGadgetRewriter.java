 ...
 */
public class AipoRenderingGadgetRewriter implements GadgetRewriter {

  // class name for logging purpose
  private static final String classname = RenderingGadgetRewriter.class
    .getName();
  ...
  public AipoRenderingGadgetRewriter(MessageBundleFactory messageBundleFactory,
      Expressions expressions, ContainerConfig containerConfig,
      FeatureRegistryProvider featureRegistryProvider,
      JsServingPipeline jsServingPipeline, JsUriManager jsUriManager,
      ConfigProcessor configProcessor, GadgetAdminStore gadgetAdminStore,
  ...
  @Inject
  public void setDefaultForcedLibs(
      @Named("shindig.gadget-rewrite.default-forced-libs") String forcedLibs) {
    if (StringUtils.isNotBlank(forcedLibs)) {
      defaultExternLibs =
        ImmutableSortedSet.copyOf(Splitter.on(':').split(forcedLibs));
    }
  }

  @Inject(optional = true)
  public void setExternalizeFeatureLibs(
      @Named("shindig.gadget-rewrite.externalize-feature-libs") Boolean externalizeFeatures) {
   ...
   *
   * @param result
   *          the target node where results should be inserted
   * @param source
   *          the source node of the template being processed
   */
  private void processNode(Node source) {
    switch (source.getNodeType()) {
      case Node.TEXT_NODE:
        try {
          source.setTextContent(String.valueOf(expressions.parse(
  ...
  public void rewrite(Gadget gadget, MutableContent mutableContent)
    ...
    try {
      ...
      MessageELResolver messageELResolver =
        new MessageELResolver(expressions, bundle);

      this.elContext =
        expressions.newELContext(messageELResolver, new GadgetELResolver(gadget
          .getContext()));
      ...
      injectedStyle.appendChild(defaultStyle.getOwnerDocument().createTextNode(
      ...
      if (gadget.getSpecificationVersion().isEqualOrGreaterThan("2.0.0")
        ...
        String value =
          containerConfig.getString(container, REWRITE_DOCTYPE_QNAME);
        if (value != null) {
          doctype_qname = value;
        }
  ...
  protected void injectDefaultScrolling(Element injectedStyle) {
    injectedStyle.appendChild(injectedStyle.getOwnerDocument().createTextNode(
      SCROLLING_CSS));
  }
  ...
  protected void injectBaseTag(Gadget gadget, Node headTag) {
    GadgetContext context = gadget.getContext();
    if (containerConfig
      .getBool(context.getContainer(), INSERT_BASE_ELEMENT_KEY)) {
   ...
   */
  protected void injectGadgetBeacon(Gadget gadget, Node headTag,
      Node firstHeadChild) throws GadgetException {
   ...
   */
  protected void injectFeatureLibraries(Gadget gadget, Node headTag,
      Node firstHeadChild) throws GadgetException {
    ...
    // Check to make sure all the required features that are about to be
    // injected are allowed
    ...
    Collection<String> gadgetLibs =
      Lists.newArrayList(gadget.getDirectFeatureDeps());
    List<Feature> gadgetFeatures =
      gadget.getSpec().getModulePrefs().getAllFeatures();
    for (Feature feature : gadgetFeatures) {
      if (!feature.getRequired()
        && !gadgetAdminStore.isAllowedFeature(feature, gadget)) {
        // If the feature is optional and the admin has not allowed it don't
        // include it
    ...
    Set<String> allLibs =
      ImmutableSet.<String> builder().addAll(externForcedLibs).addAll(
        gadgetLibs).build();
   ...
   */
  protected void checkRequiredFeatures(Gadget gadget,
      FeatureRegistry featureRegistry) throws GadgetException {
    ...
    featureRegistry.getFeatureResources(
      gadget.getContext(),
      gadgetFeatureKeys,
      unsupported).getResources();
    if (!unsupported.isEmpty()) {
      List<String> requiredUnsupported = Lists.newLinkedList();
      for (String notThere : unsupported) {
        if (!featureMap.containsKey(notThere)
          || featureMap.get(notThere).getRequired()) {
          // if !containsKey, the lib was forced with Gadget.addFeature(...) so
          // implicitly req'd.
  ...
  protected String getFeaturesContent(JsUri jsUri) throws GadgetException {
    ...
    JsRequest jsRequest =
      new JsRequestBuilder(jsUriManager, featureRegistryProvider.get(jsUri
        .getRepository())).build(jsUri, null);
   ...
   */
  protected void injectScript(Collection<String> libs,
      Collection<String> loaded, boolean inline, Gadget gadget, Node headTag,
      Node firstHeadChild, String extraContent) throws GadgetException {
  ...
  protected void injectMessageBundles(MessageBundle bundle, Node scriptTag)
    ...
    Text text =
      scriptTag
        .getOwnerDocument()
        .createTextNode("gadgets.Prefs.setMessages_(");
  ...
  protected void injectDefaultPrefs(Gadget gadget, Node scriptTag) {
    ...
    Map<String, String> defaultPrefs =
      Maps.newHashMapWithExpectedSize(prefs.size());
    for (UserPref up : prefs) {
      defaultPrefs.put(up.getName(), up.getDefaultValue());
    }
    Text text =
      scriptTag.getOwnerDocument().createTextNode(
      ...
      try {
        ...
        if (LOG.isLoggable(Level.WARNING)) {
          LOG.logp(
            ...
            classname,
            "injectPreloads",
            MessageKeys.UNEXPECTED_ERROR_PRELOADING);
