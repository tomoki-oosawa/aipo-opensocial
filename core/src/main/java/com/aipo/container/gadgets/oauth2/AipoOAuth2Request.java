 ...
 *
 * @see AipoOAuth2Request
 *
 */
public class AipoOAuth2Request implements OAuth2Request {
  private static final String LOG_CLASS = AipoOAuth2Request.class.getName();

  private static final FilteredLogger LOG = FilteredLogger
    .getFilteredLogger(AipoOAuth2Request.LOG_CLASS);
   ...
   */
  @Inject
  public AipoOAuth2Request(
      final OAuth2FetcherConfig fetcherConfig,
    ...
    if (AipoOAuth2Request.LOG.isLoggable()) {
      AipoOAuth2Request.LOG.log("this.fetcherConfig = {0}", this.fetcherConfig);
      AipoOAuth2Request.LOG.log("this.store = {0}", this.store);
      AipoOAuth2Request.LOG.log("this.fetcher = {0}", this.fetcher);
      AipoOAuth2Request.LOG.log(
        "this.authorizationEndpointResponseHandlers = {0}",
        this.authorizationEndpointResponseHandlers);
      AipoOAuth2Request.LOG.log(
        "this.clientAuthenticationHandlers = {0}",
        this.clientAuthenticationHandlers);
      AipoOAuth2Request.LOG.log(
        "this.grantRequestHandlers = {0}",
        this.grantRequestHandlers);
      AipoOAuth2Request.LOG.log(
        "this.resourceRequestHandlers = {0}",
        this.resourceRequestHandlers);
      AipoOAuth2Request.LOG.log(
        "this.tokenEndpointResponseHandlers = {0}",
        this.tokenEndpointResponseHandlers);
      AipoOAuth2Request.LOG.log(
        "this.sendTraceToClient = {0}",
  ...
  public HttpResponse fetch(final HttpRequest request) {
    final boolean isLogging = AipoOAuth2Request.LOG.isLoggable();
    if (isLogging) {
      AipoOAuth2Request.LOG.entering(
        AipoOAuth2Request.LOG_CLASS,
        "fetch",
        request);
    }
    ...
    try {
      ...
      if (request == null || request.getSecurityToken() == null) {
        ...
        response =
          this.sendErrorResponse(
            null,
            OAuth2Error.MISSING_FETCH_PARAMS,
        ...
        if (AipoOAuth2Request.LOG.isLoggable()) {
          AipoOAuth2Request.LOG.log("this.realRequest = {0}", this.realRequest);
          AipoOAuth2Request.LOG.log(
            "this.securityToken = {0}",
            this.securityToken);
          AipoOAuth2Request.LOG.log(
            "this.responseParams = {0}",
            this.responseParams);
          AipoOAuth2Request.LOG.log("this.arguments = {0}", this.arguments);
        }
        ...
        if (this.responseParams == null || this.arguments == null) {
          ...
          return this.sendErrorResponse(
            null,
            OAuth2Error.FETCH_INIT_PROBLEM,
        ...
        if (AipoOAuth2Request.LOG.isLoggable()) {
          AipoOAuth2Request.LOG.log("accessor", accessor);
        }
        ...
        if (accessor == null) {
          ...
          final Map<String, String> requestParams =
            this.requestParameterGenerator.generateParams(this.realRequest);
      ...
      if (accessor == null) {
        accessor =
          new AipoOAuth2Accessor(
            t,
            OAuth2Error.FETCH_PROBLEM,
            "exception occurred during fetch",
            "");
      } else {
        accessor.setErrorResponse(
          t,
          OAuth2Error.FETCH_PROBLEM,
          "exception occurred during fetch",
          "");
      ...
      response =
        this.processResponse(accessor, this.getErrorResponseBuilder(
          t,
          OAuth2Error.FETCH_PROBLEM,
          "exception occurred during fetch"));
    } finally {
      if (accessor != null) {
        if (!accessor.isRedirecting()) {
          if (AipoOAuth2Request.LOG.isLoggable()) {
            AipoOAuth2Request.LOG.log(
              "accessor is not redirecting, remove it",
              accessor);
          }
          accessor.invalidate();
          ...
          if (!accessor.isValid()) {
            if (AipoOAuth2Request.LOG.isLoggable()) {
              AipoOAuth2Request.LOG.log("accesssor is not valid", accessor);
            }
          } else if (accessor.isErrorResponse()) {
            if (AipoOAuth2Request.LOG.isLoggable()) {
              AipoOAuth2Request.LOG.log("accessor isErrorResponse", accessor
  ...
  private HttpResponseBuilder attemptFetch(final OAuth2Accessor accessor) {
    ...
    if (isLogging) {
      AipoOAuth2Request.LOG.entering(
        ...
        new Object[] { accessor });
      AipoOAuth2Request.LOG.log(
        "AipoOAuth2Request.haveAccessToken(accessor) = {0}",
        AipoOAuth2Request.haveAccessToken(accessor) != null);
      AipoOAuth2Request.LOG.log(
        "AipoOAuth2Request.haveRefreshToken(accessor) = {0}",
        AipoOAuth2Request.haveRefreshToken(accessor) != null);
    }

    if (this.attemptCounter > AipoOAuth2Request.MAX_ATTEMPTS) {
      if (isLogging) {
        AipoOAuth2Request.LOG.log(
          "MAX_ATTEMPTS exceeded {0}",
          this.attemptCounter);
        ...
        AipoOAuth2Request.LOG.log("MAX_ATTEMPTS stack = {0}", stack);
      }
    ...
    if (isLogging) {
      AipoOAuth2Request.LOG.log("attempt number {0}", this.attemptCounter);
    }
    ...
    if (accessor.isErrorResponse()) {
      ...
      return this.getErrorResponseBuilder(
        accessor.getErrorException(),
        accessor.getError(),
        accessor.getErrorContextMessage(),
        accessor.getErrorUri(),
        accessor.getErrorContextMessage());
    } else {
      if (AipoOAuth2Request.haveAccessToken(accessor) != null) {
        // We have an access_token, use it and stop!
        // Don't try more than three times
        ret =
          this.fetchData(
            accessor,
            this.attemptCounter > AipoOAuth2Request.MAX_ATTEMPTS);
      } else {
        // We don't have an access token, we need to try and get one.
        // First step see if we have a refresh token
        if (AipoOAuth2Request.haveRefreshToken(accessor) != null) {
          if (AipoOAuth2Request.checkCanRefresh()) {
            boolean attempt = false;
            final String internedAccessor = getAccessorKey(accessor).intern();
            if (isLogging) {
              AipoOAuth2Request.LOG.log(
                "about to synchronize on {0}",
                internedAccessor);
            }
            // This syncrhonized block is less than ideal.
            // It is needed because if a gadget has multiple makeRequests that
            // triggers
            // multiple refreshes they can end up clobbering each other, and
            // cause
            // temporary failures until the gadget is refreshed.
            // Syncrhonizing on the internedAccessor helps. It is not cluster
            // safe
            ...
            synchronized (internedAccessor) {
              ...
              if (isLogging) {
                AipoOAuth2Request.LOG.log("acc = {0}", acc);
                AipoOAuth2Request.LOG.log(
                  "AipoOAuth2Request.haveAccessToken(acc) = {0}",
                  AipoOAuth2Request.haveAccessToken(acc) == null);
                AipoOAuth2Request.LOG.log(
                  "AipoOAuth2Request.haveRefreshToken(acc) = {0}",
                  AipoOAuth2Request.haveRefreshToken(acc) == null);
              }
              if (AipoOAuth2Request.haveAccessToken(acc) != null) {
                // Another refresh must have won
                if (isLogging) {
                  AipoOAuth2Request.LOG.log(
                    "found an access token from another refresh",
                    new Object[] {});
                }
                attempt = true;
              } else {
                final OAuth2HandlerError handlerError =
                  this.refreshToken(accessor);
                if (handlerError == null) {
                  // No errors refreshing, attempt the fetch again.
                  attempt = true;
                  if (isLogging) {
                    AipoOAuth2Request.LOG.log(
                      "no refresh errors reported",
                      new Object[] {});
                  }
                } else {
                  if (isLogging) {
                    AipoOAuth2Request.LOG.log(
                      "refresh errors reported",
                      new Object[] {});
                  }
                  // There was an error refreshing, stop.
                  final OAuth2Error error = handlerError.getError();
                  ret =
                    this.getErrorResponseBuilder(
                      handlerError.getCause(),
                      error,
            ...
            if (attempt) {
              if (isLogging) {
                AipoOAuth2Request.LOG.log(
                  "going to re-attempt with a clean accesor",
                  new Object[] {});
              }
  ...
  private String authorize(final OAuth2Accessor accessor) {
    ...
    if (grantRequestHandlerUsed == null) {
      accessor.setErrorResponse(
        null,
        OAuth2Error.AUTHENTICATION_PROBLEM,
      ...
      try {
        ...
        if (isLogging) {
          AipoOAuth2Request.LOG.log("error getting complete url", e);
        }
      }
      if (grantRequestHandlerUsed.isRedirectRequired()) {
        ret = completeAuthUrl;
      } else {
        final OAuth2HandlerError error =
          this.authorize(accessor, grantRequestHandlerUsed, completeAuthUrl);
        if (error != null) {
          accessor.setErrorResponse(
            error.getCause(),
            OAuth2Error.AUTHENTICATION_PROBLEM,
            error.getContextMessage() + " , " + error.getDescription(),
            error.getUri());
        }
      }
    }
  ...
  private OAuth2HandlerError authorize(final OAuth2Accessor accessor,
      final GrantRequestHandler grantRequestHandler,
      final String completeAuthUrl) {
    ...
    if (isLogging) {
      AipoOAuth2Request.LOG.entering(
        AipoOAuth2Request.LOG_CLASS,
        "authorize",
        new Object[] { accessor, grantRequestHandler, completeAuthUrl });
    }
    ...
    try {
      authorizationRequest =
        grantRequestHandler.getAuthorizationRequest(accessor, completeAuthUrl);
    } catch (final OAuth2RequestException e) {
    ...
    if (authorizationRequest != null) {
      ...
      try {
        ...
        if (isLogging) {
          AipoOAuth2Request.LOG.log("authorize()", e);
        }
        authorizationResponse = null;
        ret =
          new OAuth2HandlerError(
            OAuth2Error.AUTHORIZE_PROBLEM,
            "exception thrown fetching authorization",
            e);
      }
      ...
      if (authorizationResponse != null) {
        if (grantRequestHandler.isAuthorizationEndpointResponse()) {
          for (final AuthorizationEndpointResponseHandler authorizationEndpointResponseHandler : this.authorizationEndpointResponseHandlers) {
            if (authorizationEndpointResponseHandler.handlesResponse(
              accessor,
              authorizationResponse)) {
              if (isLogging) {
                AipoOAuth2Request.LOG.log(
                  "using AuthorizationEndpointResponseHandler = {0}",
                  authorizationEndpointResponseHandler);
              }
              ret =
                authorizationEndpointResponseHandler.handleResponse(
                  accessor,
                  authorizationResponse);
              if (ret != null) {
        ...
        if (ret == null) {
          if (grantRequestHandler.isTokenEndpointResponse()) {
            for (final TokenEndpointResponseHandler tokenEndpointResponseHandler : this.tokenEndpointResponseHandlers) {
              if (tokenEndpointResponseHandler.handlesResponse(
                accessor,
                authorizationResponse)) {
                if (isLogging) {
                  AipoOAuth2Request.LOG.log(
                    "using TokenEndpointResponseHandler = {0}",
                    tokenEndpointResponseHandler);
                }
                ret =
                  tokenEndpointResponseHandler.handleResponse(
                    accessor,
                    authorizationResponse);
                if (ret != null) {
                  // error occurred stop processing
  ...
  private static String buildRefreshTokenUrl(final OAuth2Accessor accessor) {
    ...
    if (refreshUrl != null) {
      ret = AipoOAuth2Request.getCompleteRefreshUrl(refreshUrl);
    }
  ...
  private boolean checkCanAuthorize(final OAuth2Accessor accessor) {
    ...
    if (AipoOAuth2Request.LOG.isLoggable()) {
      AipoOAuth2Request.LOG.log("securityToken = {0}", this.securityToken);
    }
    ...
    if (AipoOAuth2Request.LOG.isLoggable()) {
      AipoOAuth2Request.LOG.log("pageOwner = {0}", pageOwner);
      AipoOAuth2Request.LOG.log("pageViewer = {0}", pageViewer);
    }
    ...
    if (pageOwner == null || pageViewer == null) {
      accessor.setErrorResponse(
        ...
        "");
      ret = false;
    } else if (!this.fetcherConfig.isViewerAccessTokensEnabled()
      && !pageOwner.equals(pageViewer)) {
      accessor.setErrorResponse(
        null,
        OAuth2Error.AUTHORIZE_PROBLEM,
        "pageViewer is not pageOwner",
        "");
      ret = false;
    }
  ...
  private HttpResponseBuilder fetchData(final OAuth2Accessor accessor,
    ...
    try {
      final HttpResponse response =
        this.fetchFromServer(accessor, this.realRequest, lastAttempt);
      if (response != null) {
      ...
      ret =
        this.getErrorResponseBuilder(e, e.getError(), e.getErrorText(), e
          .getErrorUri(), e.getErrorDescription());
    }
  ...
  private HttpResponse fetchFromServer(final OAuth2Accessor accessor,
      final HttpRequest request, final boolean lastAttempt)
        ...
        if (currentTime >= expiresAt) {
          ...
          try {
            ...
            throw new OAuth2RequestException(
              OAuth2Error.MISSING_SERVER_RESPONSE,
              "error removing access_token",
              null);
          }
          accessToken = null;
          accessor.setAccessToken(null);
    ...
    if (AipoOAuth2Request.LOG.isLoggable()) {
      AipoOAuth2Request.LOG.log("accessToken = {0}", accessToken);
      AipoOAuth2Request.LOG.log("refreshToken = {0}", refreshToken);
    }

    if (accessToken != null) {
      final boolean isAllowed =
        OAuth2Utils
          .isUriAllowed(request.getUri(), accessor.getAllowedDomains());
      if (isAllowed) {
    ...
    try {
      ...
      throw new OAuth2RequestException(
        OAuth2Error.MISSING_SERVER_RESPONSE,
        "GadgetException fetchFromServer",
        e);
    }
    ...
    if (isLogging) {
      AipoOAuth2Request.LOG.log("responseCode = {0}", responseCode);
    }
  ...
  private OAuth2Accessor getAccessorInternal() {
    ...
    if (this.fetcherConfig != null) {
      final GadgetOAuth2TokenStore tokenStore =
        this.fetcherConfig.getTokenStore();
      if (tokenStore != null) {
        ret =
          tokenStore.getOAuth2Accessor(
            this.securityToken,
            this.arguments,
  ...
  private HttpResponseBuilder getErrorResponseBuilder(final Throwable t,
  ...
  private HttpResponseBuilder getErrorResponseBuilder(final Throwable t,
      final OAuth2Error error, final String contextMessage,
      final String errorUri, final String errorDescription) {
    ...
    if (this.sendTraceToClient) {
      this.responseParams.addToResponse(ret, error.getErrorCode(), error
        .getErrorDescription(contextMessage)
        + " , "
        + errorDescription, errorUri, error.getErrorExplanation());
    } else {
      this.responseParams.addToResponse(
        ret,
        error.getErrorCode(),
        "",
        "",
  ...
  private static String getRefreshBody(final OAuth2Accessor accessor) {
    ...
    try {
      ...
      queryParams.put(OAuth2Message.REFRESH_TOKEN, new String(accessor
        .getRefreshToken()
        .getSecret(), "UTF-8"));
      ...
      if (isLogging) {
        AipoOAuth2Request.LOG.log("error generating refresh body", e);
        ret = null;
      }
    }
  ...
  private HttpResponse processResponse(final OAuth2Accessor accessor,
    ...
    if (this.responseParams.getAuthorizationUrl() != null) {
      responseBuilder.setMetadata(
        OAuth2ResponseParams.APPROVAL_URL,
  ...
  private OAuth2HandlerError refreshToken(final OAuth2Accessor accessor) {
    ...
    refershTokenUrl = AipoOAuth2Request.buildRefreshTokenUrl(accessor);

    if (isLogging) {
      AipoOAuth2Request.LOG.log("refershTokenUrl = {0}", refershTokenUrl);
    }

    if (refershTokenUrl != null) {
      HttpResponse response = null;
      final HttpRequest request = new HttpRequest(Uri.parse(refershTokenUrl));
      request.setSecurityToken(new AnonymousSecurityToken("", 0L, accessor
        .getGadgetUri()));
      ...
      for (final ClientAuthenticationHandler clientAuthenticationHandler : this.clientAuthenticationHandlers) {
        if (clientAuthenticationHandler
          .geClientAuthenticationType()
          .equalsIgnoreCase(accessor.getClientAuthenticationType())) {
          clientAuthenticationHandler
            .addOAuth2Authentication(request, accessor);
        }
      }

      try {
        final byte[] body =
          AipoOAuth2Request.getRefreshBody(accessor).getBytes("UTF-8");
        request.setPostBody(body);
      } catch (final Exception e) {
        if (isLogging) {
          AipoOAuth2Request.LOG.log("refreshToken()", e);
        }
        ret =
          new OAuth2HandlerError(
            OAuth2Error.REFRESH_TOKEN_PROBLEM,
            "error generating refresh body",
            e);
      }

      if (!OAuth2Utils.isUriAllowed(request.getUri(), accessor
        .getAllowedDomains())) {
        ret =
          new OAuth2HandlerError(
            OAuth2Error.REFRESH_TOKEN_PROBLEM,
            "error fetching refresh token - domain not allowed",
            null);
      }
      ...
      if (ret == null) {
        ...
        if (isLogging) {
          AipoOAuth2Request.LOG.log("response = {0}", response);
        }
        ...
        if (ret == null) {
          ...
          if (statusCode == HttpResponse.SC_UNAUTHORIZED
            ...
            ret =
              new OAuth2HandlerError(
                OAuth2Error.REFRESH_TOKEN_PROBLEM,
                "bad response from server : " + statusCode,
                null,
                "",
  ...
  private HttpResponse sendErrorResponse(final Throwable t,
      final OAuth2Error error, final String contextMessage) {
    final HttpResponseBuilder responseBuilder =
      this.getErrorResponseBuilder(t, error, contextMessage);
  ...
  private static OAuth2Token haveAccessToken(final OAuth2Accessor accessor) {
    ...
    if (ret != null) {
      if (!AipoOAuth2Request.validateAccessToken(ret)) {
        ret = null;
      }
    }
    return ret;
  ...
  private static OAuth2Token haveRefreshToken(final OAuth2Accessor accessor) {
    ...
    if (ret != null) {
      if (!AipoOAuth2Request.validateRefreshToken(ret)) {
        ret = null;
      }
    }
    return ret;
